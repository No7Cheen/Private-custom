...
public abstract class SDSQLiteOpenHelper extends SQLiteOpenHelper {
	...
	/** 日志标记. */
	protected final String TAG = "SDSQLiteOpenHelper";
		...
		try {
			...
			try {
				...
				if (db.getVersion() != mNewVersion) {
					throw new SQLiteException("不能更新只读数据库的版本 from version " + db.getVersion() + " to " + mNewVersion
							+ ": " + path);
				}
				onOpen(db);
				mDatabase = db;
	...
	public File getDatabasePath(String dbpath, String dbName) {
		dbpath = StringUtils.parseEmpty(dbpath);

		// 创建目录
		File path = new File(Environment.getExternalStorageDirectory() + File.separator + dbpath);
		// 创建文件
