...
import com.pc.graphics.util.RectUtils;
import com.pc.utils.math.PcMathUtil;

public class KenBurnsViewRandomTransitionGenerator implements KenBurnsViewTransitionGenerator {
	...
	private KenBurnsViewTransition mLastGenTrans;

	/** The bounds of the drawable when the last transition was generated. */
	private RectF mLastDrawableBounds;

	public KenBurnsViewRandomTransitionGenerator() {
		this(DEFAULT_TRANSITION_DURATION, new AccelerateDecelerateInterpolator());
	}

	public KenBurnsViewRandomTransitionGenerator(long transitionDuration, Interpolator transitionInterpolator) {
		setTransitionDuration(transitionDuration);
		setTransitionInterpolator(transitionInterpolator);
	}

	@Override
	public KenBurnsViewTransition generateNextTransition(RectF drawableBounds, RectF viewport) {
		RectF srcRect;
		...
		mLastGenTrans = new KenBurnsViewTransition(srcRect, dstRect, mTransitionDuration, mTransitionInterpolator);
	...
	private RectF generateRandomRect(RectF drawableBounds, RectF viewportRect) {
		float drawableRatio = RectUtils.getRectRatio(drawableBounds);
		float viewportRectRatio = RectUtils.getRectRatio(viewportRect);
		RectF maxCrop;
		...
		float randomFloat = PcMathUtil.truncate(mRandom.nextFloat(), 2);
