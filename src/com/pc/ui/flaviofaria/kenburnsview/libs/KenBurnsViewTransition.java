...
public class KenBurnsViewTransition {
	...
	public KenBurnsViewTransition(RectF srcRect, RectF dstRect, long duration, Interpolator interpolator) {
		// Reduces precision to avoid problems when comparing aspect ratios.
		float srcRectRatio = PcMathUtil.truncate(RectUtils.getRectRatio(srcRect), 2);
		float dstRectRatio = PcMathUtil.truncate(RectUtils.getRectRatio(dstRect), 2);
