...
public class TableProperty {
	...
	private static TableProperty mAbTable = null;
	...
	public TableProperty(Context context) {
		mScreenWidth = TerminalUtils.terminalW(context);
	}
	 ...
	 * @return the ab table
	 */
	public static TableProperty newAbTable(Context context, int columnSize) {
		if (columnSize <= 0) {
			// Toast.makeText(context, "请设置表格的列数>0!",
			// Toast.LENGTH_SHORT).show();
			return null;
		}

		mAbTable = new TableProperty(context);
		...
		// 列类型配置
		mAbTable.cellTypes = new EmCellType[columnSize];
		for (int i = 0; i < columnSize; i++) {
			mAbTable.cellTypes[i] = EmCellType.type_string;
		}

		// 列宽配置(%)
		mAbTable.cellWidth = new int[columnSize];
		for (int i = 0; i < columnSize; i++) {
			mAbTable.cellWidth[i] = mAbTable.mScreenWidth / columnSize;
		}

		// 行高配置（索引0标题，1内容列表）
		mAbTable.rowHeight = new int[] {
				30, 30
		};

		// 行文字大小（索引0标题，1内容列表）
		mAbTable.rowTextSize = new int[] {
				18, 16
		};

		// 行文字颜色（索引0标题，1内容列表）
		mAbTable.rowTextColor = new int[] {
				Color.rgb(113, 113, 113), Color.rgb(113, 113, 113)
		};

		// 背景资源（索引0标题行背景，1标题单元格背景，2内容列表行背景。3表格内容单元格背景）
		mAbTable.tableResource = new int[] {
	 ...
	 */
	public EmCellType[] getCellTypes() {
		return cellTypes;
	}
	 ...
	 * @param cellTypes 列类型
	 */
	public void setCellTypes(EmCellType[] cellTypes) {
	...
	/**
	 * 表格的背景资源（索引0标题行背景，1标题单元格背景，2内容列表行背景。3表格内容单元格背景）.
