...
public class PcMonthItemView extends View {
	...
	/** week labels */
	private final String[] mWeekLabelValues = Arrays.copyOfRange((DateFormatSymbols.getInstance().getShortWeekdays()), 1, 8);
	/** month labels */
	private final String[] mMonthLabelValues = Arrays.copyOfRange((DateFormatSymbols.getInstance().getShortMonths()), 0, 12);
	...
	public PcMonthItemView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public PcMonthItemView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (attrs != null) {
			...
			mSelectedDayColor = a.getColor(R.styleable.CalendarStyle_selecteColor, mSelectedDayColor);
	 ...
	 * @param currYear今天坐在年
	 */
	public void setParams(Calendar adaptCal, Calendar selectCal, int year, int month, int today, int currMonth, int currYear) {
		...
		for (DayEntity entity : mValues) {
			...
			boolean selected = mSelectedCalendar.get(Calendar.YEAR) == y && mSelectedCalendar.get(Calendar.MONTH) == m && mSelectedCalendar.get(Calendar.DAY_OF_MONTH) == d;
	...
	private Calendar getDayByLocation(float x, float y) {
		...
		mLocalVisibleRect.set(mLocalVisibleRect.left, mLocalVisibleRect.top + mWeekLabelHeight, mLocalVisibleRect.right, mLocalVisibleRect.bottom);
