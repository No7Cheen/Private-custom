/*
 * Copyright (C) 2013 47 Degrees, LLC http://47deg.com hello@47deg.com Copyright
 * 2012 Roman Nurik Licensed under the Apache License, Version 2.0 (the
...
package com.pc.ui.fortysevendeg.swipelistview;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private boolean swipeOpenSingle = true;
	...
	/**
	 * open only one back view
	 * @param swipeOpenSingle
	 */
	public void setWwipeOpenSingle(boolean swipeOpenSingle) {
		this.swipeOpenSingle = swipeOpenSingle;
	}
		...
		switch (MotionEventCompat.getActionMasked(motionEvent)) {
			...
			case MotionEvent.ACTION_UP: {
				...
				// }
					...
					swapRight = deltaX > 0;
					if (swapRight) {
						if (Math.abs(deltaX) >= (viewWidth - rightOffset) / 2) {
							swap = true;
						}
					} else {
						if (Math.abs(deltaX) >= (viewWidth - leftOffset) / 2) {
							swap = true;
						}
					}
