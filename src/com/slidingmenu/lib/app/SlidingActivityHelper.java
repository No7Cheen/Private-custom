...
public class SlidingActivityHelper {
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		...
		if (null == mSlidingMenu) return false;

		if (null != customOnKeyup) {
			return customOnKeyup.onKeyUp(keyCode, event);
		}

		if (isShowContent4BackKey) {
			if (keyCode == KeyEvent.KEYCODE_BACK && mSlidingMenu.isMenuShowing()) {
				showContent();
				return true;
			}
	...
	/**
	 * 是否使用back键显示Content, false:show menu
	 */
	private boolean isShowContent4BackKey = true;
	private IOnKeyup customOnKeyup;

	public void setShowContent4BackKey(boolean isShowContent4BackKey) {
		this.isShowContent4BackKey = isShowContent4BackKey;
	}

	public void setCustomOnKeyup(IOnKeyup customOnKeyup) {
		this.customOnKeyup = customOnKeyup;
	}
